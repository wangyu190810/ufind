...
def get_user_detail_info():
    if request.method == "GET":
        ...
        if user_info.active == 1 and (user_info.account != 0 or user_info.active is not None):
            coupons["code"] = user_info.coupon
            coupons["account"] = user_info.account
            student_info["coupons"] = coupons
        elif user_info.active == 2 and (user_info.account != 0 or user_info.active is not None):
            coupons["code"] = None
            coupons["account"] = user_info.account
            student_info["coupons"] = coupons
